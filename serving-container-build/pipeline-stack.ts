...
export class PipelineStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
      ...
      const BASE_IMAGE_AMD_XLA_TAG = new CfnParameter(
          this,
          'BASEIMAGEAMDXLATAG',
          { type: 'String' }
      );
      const BASE_IMAGE_AMD_CUD_TAG = new CfnParameter(
          this,
          'BASEIMAGEAMDCUDTAG',
          { type: 'String' }
      );
      const BASE_IMAGE_ARM_CPU_TAG = new CfnParameter(
          this,
          'BASEIMAGEARMCPUTAG',
          { type: 'String' }
      );
      const IMAGE_AMD_XLA_TAG = new CfnParameter(this, 'IMAGEAMDXLATAG', {
          type: 'String',
      });
      const IMAGE_AMD_CUD_TAG = new CfnParameter(this, 'IMAGEAMDCUDTAG', {
          type: 'String',
      });
      const IMAGE_ARM_CPU_TAG = new CfnParameter(this, 'IMAGEARMCPUTAG', {
          type: 'String',
      });
      const GITHUB_OAUTH_TOKEN = new CfnParameter(this, 'GITHUBOAUTHTOKEN', {
          type: 'String',
      });
      const GITHUB_USER = new CfnParameter(this, 'GITHUBUSER', {
          type: 'String',
      });
      const GITHUB_REPO = new CfnParameter(this, 'GITHUBREPO', {
          type: 'String',
      });
      const GITHUB_BRANCH = new CfnParameter(this, 'GITHUBBRANCH', {
          type: 'String',
      });
      ...
      const githubSecret = new secretsmanager.Secret(this, 'githubSecret', {
          secretObjectValue: {
              token: SecretValue.unsafePlainText(
                  GITHUB_OAUTH_TOKEN.valueAsString
              ),
          },
      });
      const githubOAuthToken = SecretValue.secretsManager(
      ...
      new cdk.CfnOutput(this, 'githubOAuthTokenRuntimeOutput2', {
          value: SecretValue.secretsManager(githubSecret.secretArn, {
              jsonField: 'token',
          }).toString(),
      });

      const base_image_amd_cud_build = new codebuild.Project(
          this,
          `ImageCudAmdBuild`,
          {
              environment: {
      ...
      const base_image_arm_cpu_build = new codebuild.Project(
          this,
          `ImageCpuArmBuild`,
          {
              environment: {
                  ...
                  phases: {
                      build: {
                          commands: [
                              ...
                              `export AWS_REGION="${this.region}"`,
                              `export BASE_REPO="${BASE_REPO.valueAsString}"`,
                              `export IMAGE_TAG="${IMAGE_ARM_CPU_TAG.valueAsString}"`,
                              `export BASE_IMAGE_TAG="${BASE_IMAGE_ARM_CPU_TAG.valueAsString}"`,
                              `cd app`,
                              `chmod +x ./build.sh && ./build.sh`,
                          ],
                      },
      ...
      const base_image_amd_xla_build = new codebuild.Project(
          this,
          `ImageXlaAmdBuild`,
          {
              environment: {
      ...
      const assets_image_xla_amd_build = new codebuild.Project(
          this,
          `AssetsImageXlaAmdBuild`,
          {
              environment: {
                  privileged: true,
                  buildImage: codebuild.LinuxBuildImage.AMAZON_LINUX_2_3,
      ...
      const assets_image_cpu_arm_build = new codebuild.Project(
          this,
          `AssetsImageCpuArmBuild`,
          {
              environment: {
                  privileged: true,
                  buildImage: codebuild.LinuxBuildImage.AMAZON_LINUX_2_ARM_2,
      ...
      const assets_image_cud_amd_build = new codebuild.Project(
          this,
          `AssetsImageCudAmdBuild`,
          {
              environment: {
                  privileged: true,
                  buildImage: codebuild.LinuxBuildImage.AMAZON_LINUX_2_3,
      ...
      base_registry.grantPullPush(assets_image_cpu_arm_build.grantPrincipal);
      ...
      base_registry.grantPullPush(base_image_arm_cpu_build.grantPrincipal);

      // here we define our pipeline and put together the assembly line
      const sourceOutput = new codepipeline.Artifact();
      const basebuildpipeline = new codepipeline.Pipeline(
          this,
          `BuildBasePipeline`
      );
      ...
      basebuildpipeline.node.addDependency(githubSecret);
      ...
      basebuildpipeline.addStage({
          ...
          actions: [
              ...
              new codepipeline_actions.CodeBuildAction({
                  actionName: 'AssetsImageCpuArmBuild',
                  input: sourceOutput,
                  runOrder: 1,
                  project: assets_image_cpu_arm_build,
              }),
              new codepipeline_actions.CodeBuildAction({
              ...
              new codepipeline_actions.CodeBuildAction({
                  actionName: 'BaseImageArmCpuBuild',
                  input: sourceOutput,
                  runOrder: 2,
                  project: base_image_arm_cpu_build,
              }),
              new codepipeline_actions.CodeBuildAction({
